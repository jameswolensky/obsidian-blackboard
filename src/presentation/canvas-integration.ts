...
// The Obsidian Canvas plugin is not part of the public typings, so we describe the exact
// internals we touch with minimal local interfaces and cast the untyped view once.
...
interface Canvas {
  ...
  nodes: Map<unknown, CanvasNode>;
  requestSave(): void;
  getViewportBBox?(): CanvasViewportBBox | undefined;
  createFileNode(opts: {
...
export async function embedDrawingIntoHost(
  ...
  const view = leaf?.view as unknown as CanvasView | undefined;
  if (view?.canvas) {
